...
public final class Test<T> {
	...
	public Test(final TestSuite suite, final ISerializer<T> serializerToTest, final boolean gzipEnabled) {
		this.suite = suite;
	...
	private final void run(final ISerializer<T> serializer, final int iterations)
	{
		...
		// one iteration for warm up (JIT compiler etc) and writing raw bytes to file for logging
		...
		suite.getLogger().log(serializer.getName(), raw.length, writeTimes, readTimes);		
	}
	...
	private TestResult<byte[]> performWriteTest() {
		return time(data -> serialize(data), suite.createTestData(serializer.getType()));
	}
	...
	private <Out, In> TestResult<Out> time(final Function<In, Out> func, final In input)
	{
